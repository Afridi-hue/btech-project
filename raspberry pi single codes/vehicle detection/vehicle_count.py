...
for ab in image_data:
    ...
    for i in indices:
        ...
        # print(class_ids[i])
        # id of person is 0
        if class_ids[i]==2 or class_ids[i]==3 or class_ids[i]==5 or class_ids[i]==7:
            # print("vehicle found")
            counter+=1
            draw_prediction(image, class_ids[i], confidences[i], round(x), round(y), round(x+w), round(y+h))

    cv2.putText(image,str(counter),(100,100),cv2.FONT_HERSHEY_COMPLEX,3,(255,0,0),3) # Print the vehicle's count in the image
